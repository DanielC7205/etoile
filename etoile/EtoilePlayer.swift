...
class EtoilePlayer {
    ...
    fileprivate func tellThatWerePlaying() {
        ...
        // Clearing external
        ConfigurationSingleton.shared.externalData = nil
        
        // Updating the background color
        guard let artworkData = ConfigurationSingleton.shared.player.playingSong?.art else { return }
        guard let art = UIImage(data: artworkData) else { return }
        guard let averageColor = art.averageColor else { return }
        ConfigurationSingleton.shared.backgroundColors = [averageColor]
